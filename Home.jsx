...
export default function Home() {
  ...
  const sendDataToFlask = () => {
    if (selectedOption) {
      ...
      axios.post('http://127.0.0.1:5000/chip', { data: message })
        .then(response => {
      ...
      >
        ...
        >
          ...
          <ThemeProvider theme={theme}>
            <Box>
            {/* <div>
              <button onClick={sendDataToFlask}>发送数据给Flask</button>
            </div> */}
              <Button color="secondary" variant="contained" size="medium" href="/ra" onClick={sendDataToFlask} disabled={!selectedOption}>
                START
              </Button>
            </Box>
          </ThemeProvider>
